...
const TOKENS = [{ Token: '$ERC20AlphaMock' }, { Token: '$ERC20BetaMock' }, { Token: '$ERC20GammaMock' }];
  ...
  for (const { Token, forcedApproval } of TOKENS) {
    describe(Token, function () {
      ...
      describe('_mint', function () {
        ...
        // hashless-tokens: changed
        it('rejects overflow', async function () {
          await expect(this.token.$_mint(this.recipient, ethers.MaxUint256))
          .to.be.revertedWithCustomError(this.token, 'ERC20TotalSupplyOverflow')
          .withArgs(this.recipient, initialSupply, ethers.MaxUint256);
        });
